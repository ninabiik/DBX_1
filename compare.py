...
class Compare:
                ...
                if firstItemKPI == "":
                    ...
                    for kpi in kpis_lst:
                        ...
                        if kpi_in_first and kpi_in_second:
                            self.compare_data(company, kpi, json_first_response_dict[kpi],
                                              json_second_response_dict[kpi])
                        else:
                            if not kpi_in_first:
                                msg = "{} key not found in {} ".format(kpi, self.firstenv)
                            if not kpi_in_second:
                                msg = "{} response cannot be found in {}. Kindly check also the payload, it might be the kpi is not included.".format(
                                    kpi, self.secondenv)
    ...
    def writeToExcel(self, datalist, filename, kpi, is_segment=False):
        ...
        filename = "comparison_results/{}-{}-{}.xlsx".format(filename, self.firstenv, self.secondenv)
                ...
                for ca in data2:
                    if 'companyId' in da and da['companyId'] == ca['companyId']:
                        try:
                            ...
                            msg = '{} field is not found either on the {} or {}'.format(str(e), self.firstenv,
                                                                                        self.secondenv)
                        ...
                        try:
                            ...
                            msg = '{} field is not found either on the {} or {}'.format(str(e), self.firstenv,
                                                                                        self.secondenv)
                            mismatch = [
                                [company, da['xf_company_id'], kpi_name, str(e), 'N/A', 'N/A', 'N/A', 'N/A', msg]]
                            output.append(mismatch)
        else:
